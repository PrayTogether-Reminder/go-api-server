...
func TestDeletePrayerTitle_Success(t *testing.T) {
	...
	prayerTitle := testutil2.CreateTestPrayerTitle(t, db, testRoom.ID, "Prayer Title to Delete")
...
func TestDeletePrayerTitle_MemberNotInRoom(t *testing.T) {
	...
	prayerTitle := testutil2.CreateTestPrayerTitle(t, db, roomOwnedByAnother.ID, "Prayer Title to Delete")
...
func TestDeletePrayerTitle_CascadeDelete(t *testing.T) {
	...
	// Given: Create a prayer title
	prayerTitle := testutil2.CreateTestPrayerTitle(t, db, testRoom.ID, "Prayer Title with Contents")

	// Given: Create prayer contents associated with the prayer title
	writer := testutil2.CreateTestMemberWithIndex(t, db, 1)
	memberID1 := int64(1)
	memberID2 := int64(2)
	prayerContent1 := testutil2.CreateTestPrayerContent(t, db, prayerTitle.ID, writer.ID, writer.Name, &memberID1, "Member 1", "First Content")
	prayerContent2 := testutil2.CreateTestPrayerContent(t, db, prayerTitle.ID, writer.ID, writer.Name, &memberID2, "Member 2", "Second Content")
