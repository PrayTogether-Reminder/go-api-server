...
func TestFetchPrayerContents_Success(t *testing.T) {
	...
	prayerTitle := testutil2.CreateTestPrayerTitle(t, db, testRoom.ID, "Please pray for my family")
	router := setupPrayerContentFetchRouter(memberID, prayerHandler)

	// Given: Create test prayer contents
	member1 := testutil2.CreateTestMemberWithIndex(t, db, 1)
	member2 := testutil2.CreateTestMemberWithIndex(t, db, 2)
	targetMemberID := int64(12345)

	content1 := testutil2.CreateTestPrayerContent(
		t, db, prayerTitle.ID, memberID, "Test_User_",
		&targetMemberID, "John Doe", "First prayer content",
	)
	content2 := testutil2.CreateTestPrayerContent(
		t, db, prayerTitle.ID, member1.ID, member1.Name,
		nil, "Jane Smith", "Second prayer content",
	)
	content3 := testutil2.CreateTestPrayerContent(
...
func TestFetchPrayerContents_MemberNotInRoom(t *testing.T) {
	...
	prayerTitle := testutil2.CreateTestPrayerTitle(t, db, roomOwnedByAnother.ID, "Please pray for my family")
	router := setupPrayerContentFetchRouter(memberID, prayerHandler)

	// Given: Create prayer content in the other member's room
	testutil2.CreateTestPrayerContent(
